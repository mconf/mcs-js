...
exports.PublishAndSubscribeParams = function (test) {
  ...
  test.equal(publishAndSubscribe.body.source_id, 'M53al0p', 'Valid source_id');
  test.equal(publishAndSubscribe.body.media_id, 'default', 'Valid media_id');
  test.equal(publishAndSubscribe.body.type, 'RTP', 'Valid media_id');
  test.equal(publishAndSubscribe.body.params.descriptor, sdp, 'Valid SDP');
  test.done();
}
...
exports.PublishedAndSubscribedParams = function (test) {
  var mediaId = 'UnIqUe_Id';
  var sdp = 'v=0 o=jdoe 2890844526 2890842807 IN IP4 10.47.16.5.....'
  var publishedAndSubscribed = new PublishedAndSubscribed(mediaId, sdp);
  test.equal(publishedAndSubscribed.body.media_id, mediaId, 'Valid Media Id');
