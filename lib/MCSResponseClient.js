...
class MCSResponseClient extends MCSBaseClient {
  ...
  mediaConnected (mediaId, userId, name, muted) {
    if (!mediaId || typeof(mediaId) !== 'string') {
      throw new Error ('Error : Invalid Media ID');
    }
    ...
    let mediaConnectedMessage = new MediaConnected(mediaId, userId, name, muted);
    if (mediaConnectedMessage) {
      this.send(mediaConnectedMessage);
    }
  }

  mediaDisconnected (mediaId, userId) {
    if (!mediaId || typeof(mediaId) !== 'string') {
      throw new Error ('Error : Invalid Media ID');
    }
    ...
    let mediaDisconnectedMessage = new MediaDisconnected(mediaId, userId);
