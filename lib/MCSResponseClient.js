...
const Consumed = require('./messages/consumed.js');
const IceRestarted = require('./messages/iceRestarted.js');
...
class MCSResponseClient extends MCSBaseClient {
   ...
   */
  connected(sourceId, sinkIds, remoteDescriptor, params) {
    ...
    let connectedMessage = new Connected(sourceId, sinkIds, params);
    if (connectedMessage) {
   ...
   * TODO docs
   */
  roomCreated (room, options = {}) {
    if (!room || typeof (room) !== 'string') {
      throw (new Error('invalid room info'));
    }

    const message = new RoomCreated(room, options);
   ...
   * TODO docs
   */
  roomDestroyed (roomId, options = {}) {
    if (!roomId || typeof (roomId) !== 'string') {
      throw (new Error('invalid roomId'));
    }

    const message = new RoomDestroyed(roomId, options);
  ...
  consumed (sourceMediaId, sinkMediaId, remoteDescriptor, params) {
    if (!sourceMediaId || typeof (sourceMediaId) !== 'string') {
      throw new Error('Error : Invalid Source Media ID');
    }
    if (!sinkMediaId || typeof (sinkMediaId) !== 'string') {
      throw new Error('Error : Invalid Sink Media ID');
    }
  ...
  /**
   * @param  {String} mediaId The mediaSessionId/mediaId which is undergoing an
   * ICE restart
   * @param  {String} descriptor The local (server) descriptor with updated ICE
   * parameters
   */
  iceRestarted (mediaId, descriptor, options) {
    if (!mediaId || typeof(mediaId) !== 'string') {
      throw new Error('Error: invalid mediaId');
    }
    ...
    const message = new IceRestarted(mediaId, descriptor, options);
