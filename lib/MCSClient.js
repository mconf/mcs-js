...
const Consume = require('./messages/consume.js');
const CreateRoom = require('./messages/createRoom.js');
const DestroyRoom = require('./messages/destroyRoom.js');
const RestartIce = require('./messages/restartIce.js');
...
class MCSClient extends MCSBaseClient {
  ...
  createRoom (options = {}) {
    if (options == null || typeof(options) !== 'object') {
      throw new Error('Error: Invalid options type (createRoom)');
    }

    const message = new CreateRoom(options);
    if (message) {
  ...
  /**
   * Destroy a MCS room
   * @param  {String}   roomId  The id of the room to be destroyed
   */
  destroyRoom (roomId) {
    ...
    const message = new DestroyRoom(roomId);
    if (message) {
      return this.deferTransaction(message);
    }
  }
  ...
  consume (sourceMediaId, sinkMediaId, type = 'ALL') {
    if (!sourceMediaId || typeof(sourceMediaId) !== 'string') {
      throw new Error('Invalid connect sourceMediaId');
    }

    if (sinkMediaId == null || typeof(sinkMediaId) !== 'string') {
      throw new Error('Invalid connect sinkMediaId');
    }

    if (type == null || typeof type !== 'string') {
        throw new Error('Error: Invalid consume type');
    }

    const consume = new Consume(sourceMediaId, sinkMediaId, type);
    if (consume) {
      return this.deferTransaction(consume);
    }
  }

  /**
   * @param  {String} mediaId The mediaSessionId/mediaId to restart ICE
   */
  restartIce (mediaId) {
    if (!mediaId || typeof mediaId !== 'string') {
      throw new Error('Error: invalid identifier');
    ...
    const message = new RestartIce(mediaId);
