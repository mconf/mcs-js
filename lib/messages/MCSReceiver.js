...
class MCSReceiver extends EventEmitter {
  ...
  handleMessage(message) {
    ...
    if ( pMessage && typeof(pMessage) === 'object' &&
      ...
      // Checks for message type to either resolve or reject the method promise
      if (pMessage.name === 'errorMessage') {
        MCSTransactionManager.rejectTransaction(transactionId, pMessage.body);
      } else {
